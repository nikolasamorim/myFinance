/*
  # Create RPC Functions for Invoice Module

  1. Functions
    - `compute_statement_window` - Calculate statement period dates
    - `ensure_open_statement` - Create or return open statement
    - `sync_item_from_transaction` - Auto-attach transactions to statements
    - `close_statement` - Close statement and calculate totals
    - `register_statement_payment` - Record payment and update status
    - `move_item_to_next_cycle` - Move item to next statement period
*/

-- Function to compute statement window based on card closing/due days
CREATE OR REPLACE FUNCTION compute_statement_window(anchor_date date, card_id uuid)
...
-- Function to ensure open statement exists
...
BEGIN
    ...
    WHERE credit_card_id = card_id AND period_start = ensure_open_statement.period_start;
    ...
    FROM compute_statement_window(ensure_open_statement.period_start, card_id) cw;
    ...
    AND cs.period_end < ensure_open_statement.period_start
    ...
    INSERT INTO card_statements (
        ...
        workspace_id_val, card_id, ensure_open_statement.period_start, period_end_val, due_date_val,
...
-- Function to sync transaction to statement item
...
BEGIN
    ...
    SELECT period_start INTO window_start
    FROM compute_statement_window(tx_record.transaction_date, tx_record.transaction_card_id) 
    LIMIT 1;
    
    SELECT ensure_open_statement(tx_record.transaction_card_id, window_start) INTO statement_id;
...
-- Function to register statement payment
...
-- Function to move item to next cycle
