...
function ViewDropdownItem({ visualization, isActive, onSelect, onEdit, onDelete }: ViewDropdownItemProps) {
  ...
  return (
    <div className={cn(
      'group flex items-center justify-between px-3 py-2 hover:bg-bg-elevated transition-colors',
  ...
  return (
    <>
      ...
      <button
        ...
        {...getReferenceProps()}
        className="flex items-center space-x-2 px-3 py-1.5 text-sm bg-bg-page border border-border rounded-md shadow-sm hover:bg-bg-elevated focus:outline-none focus:ring-1 focus:ring-accent focus:border-transparent transition-colors min-w-40"
      >
      ...
      {isOpen && (
        <FloatingPortal>
          <div
            ...
            {...getFloatingProps()}
            className="bg-bg-page border border-border rounded-lg shadow-lg z-50 w-80 max-h-96 overflow-hidden"
          >
            {/* Header */}
            <div className="px-3 py-2 border-b border-border bg-bg-surface">
              <h3 className="text-sm font-medium text-text-primary">Visualizações</h3>
            </div>
