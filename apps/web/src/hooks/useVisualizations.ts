...
export function useVisualizations(screenContext: string) {
  ...
  return useQuery({
    ...
    queryFn: () => visualizationService.getUserVisualizations(currentWorkspace!.workspace_id, screenContext, user!.id),
    enabled: !!currentWorkspace?.workspace_id && !!user,
...
export function useDefaultVisualization(screenContext: string) {
  ...
  return useQuery({
    ...
    queryFn: () => visualizationService.getDefaultVisualization(currentWorkspace!.workspace_id, screenContext, user!.id),
    enabled: !!currentWorkspace?.workspace_id && !!user,
    staleTime: 5 * 60 * 1000, // 5 minutes
    retry: 1,
  });
...
export function useCreateVisualization() {
  ...
  return useMutation({
    mutationFn: (visualization: Omit<Visualization, 'visualization_id' | 'visualization_created_at' | 'visualization_updated_at'>) =>
      visualizationService.createVisualization(visualization, user!.id),
...
export function useSetDefaultVisualization() {
  ...
  return useMutation({
    mutationFn: ({ screenContext, visualizationId }: { screenContext: string; visualizationId: string }) =>
      visualizationService.setDefaultVisualization(currentWorkspace!.workspace_id, screenContext, visualizationId, user!.id),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({
